...
int main() {
    ...
    cal(LU_sem_tri);
    sum9+=(tail - head) * 1000 / freq;
    deleteA();

    cal(sem_triplecircle);
    sum10+=(tail - head) * 1000 / freq;
    deleteA();

    cal(LU_barrier);
    sum11+=(tail - head) * 1000 / freq;
    deleteA();

    cal(barrier_static);
    sum12+=(tail - head) * 1000 / freq;
    deleteA();
    }
    cout << "平凡算法串行耗时：" << sum1/ 10<<"ms" << endl;
    cout << "avx优化串行耗时： " << sum2/10 << "ms" << endl;
    cout << "平凡pthread耗时：" << sum3/10 << "ms" << endl;
    cout << "avx优化pthread耗时：" << sum4/10<< "ms" << endl;
    cout << "平凡算法pthread动态8线程： " <<sum5/10 << "ms" << endl;
    cout << "avx优化pthread动态8线程： " << sum6/10 << "ms" << endl;
    cout << "平凡算法静态8线程+信号量： " << sum7/10<< "ms" << endl;
    cout << "avx静态8线程+信号量： " << sum8/10 << "ms" << endl;
    cout << "平凡算法静态8线程+信号量三重循环： " << sum9/10 << "ms" << endl;
    cout << "avx静态8线程+信号量三重循环： " << sum10/10 << "ms" << endl;
    cout << "平凡算法静态barrier： " << sum11/10 << "ms" << endl;
    cout << "avx静态barrier：" << sum12/10 << "ms" << endl;
